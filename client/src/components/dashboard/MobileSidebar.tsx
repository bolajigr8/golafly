...
export function MobileSidebar({ open, onClose }: MobileSidebarProps) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className='flex items-center justify-between h-16 px-5 border-b border-slate-200 dark:border-white/8'>
          <Logo size='md' />
          <button
            onClick={onClose}
            className='flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-slate-900 hover:bg-slate-100 dark:text-white/60 dark:hover:text-white dark:hover:bg-white/10 transition-all'
          ...
          {navItems.map(({ label, icon: Icon, href, exact }) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  'flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium transition-all duration-150 border-l-2 pl-2.5',
                  active
